...
mamba env create -f StarDist_Crick_Sept24.yml
ml purge 
conda activate StarDist_Crick_Sept24
...
python -m ipykernel install --user --name=StarDist_Crick_Sept24
echo "Setting up juypter"
python3 -m remote_ikernel manage --add --kernel_cmd="ml purge && ml cuDNN/8.4.1.50-CUDA-11.7.0 && conda activate StarDist_Crick_Sept24 && ipython3 kernel -f {connection_file}" --name="StarDist_Crick_Sept24" --interface=local --workdir="~/"--language=python3
